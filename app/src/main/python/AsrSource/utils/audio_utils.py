...
def load_audio_files(paths: str | List[str], sampling_rate: int = 16000):
    ...
    initial_path = str(Environment.getExternalStorageDirectory()) + "/Recordings/"
    if is_backend_available(Backends.LIBROSA):
        ...
        inputs = []
        for path in paths:
            FFmpegKit.execute("-y -i " + path + f" -f s16le -ac 1 -ar {sampling_rate} -acodec pcm_s16le " + initial_path + "output.raw")
            with open(initial_path + "output.raw", "rb") as in_file:
                inputs.append(np.asarray(buf_to_float(in_file.read())))
    else:
